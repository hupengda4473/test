	...
	export default {
		...
		methods: {
			...
			async chooseClick() {
				...
				uni.chooseImage({
					...
					success: async function(res) {
						...
						var checkImageRes = await that.checkImage(res.tempFilePaths[0])
						console.log('checkImageRes', checkImageRes)
						if (!checkImageRes) {
							return
						}
			...
			async checkImage(path) {
				...
				var cloudPath = 'checkImage' + '/' + year + '/' + month + '/' + day + '/' + hours + minutes + seconds +
					'_' + random + '.' + extension
				await uniCloud.uploadFile({
					...
					let sec_check = await uniCloud.callFunction({
						name: 'sec_check',
						data: {
							"checkType": "image",
							"checkContent": {
					...
					uniCloud
					  ...
					  .then(res => {});
					console.log('sec_check', sec_check.result.errCode)
					uni.hideLoading()
					if(sec_check.result.errCode!=0){
						that.$u.toast("图片审核不通过");
						checkRes = false
					}
