<template>
	<view>
		...
		<view class="" style="padding-top: 80rpx;">
			<u-search @clear="getUserList(1)" @search="searchClick" placeholder="输入搜索内容" shape="round" v-model="searchKeyWord" :show-action="false" :animation="true"></u-search>
			...
			async getUserList(on){
				...
				let list = await uniCloud.callFunction({
					...
					data: {
						...
						searchMore: that.searchKeyWord.trim()?{'name': that.searchKeyWord.trim()}:'',
						searchMoreType: 'and',
						pageSize: 15
					},
				})
