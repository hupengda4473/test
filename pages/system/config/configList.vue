<template>
	<view  v-if="vuex_user.system == 1">
		<u-search @clear="getConfigList" @search="searchClick" placeholder="输入搜索内容" shape="round" v-model="searchKeyWord" :show-action="false" :animation="true"></u-search>
			...
			async getConfigList(){
				...
				let list = await uniCloud.callFunction({
					...
					data: {
						...
						pageIndex: 1,
						pageSize: 100,
						searchMore: that.searchKeyWord.trim()?{'name': that.searchKeyWord.trim(), 'key': that.searchKeyWord.trim()}:'',
						searchMoreType: 'or'
					},
				})
