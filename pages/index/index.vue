<template>
	<view>
		<view class="" v-show="!loadShow">
			...
			<view class="content u-p-t-20">
				<view class="banner">
					<u-swiper :effect3d="true" width="100%" height="300" name="image" border-radius="10"
				...
				<view class="aui-palace" v-if="!vuex_isShenHe&&vuex_indexUiType==0">
					...
					<view @click="taskClick()" class="aui-palace-grid">
						<view class="aui-palace-grid-icon">
				...
				<view v-if="!vuex_isShenHe&&vuex_indexUiType==1">
					<view>
						<view class="u-p-t-40 u-p-l-30 u-p-r-30">
				...
				<view class="u-p-t-20 u-p-l-30 u-p-r-30" v-if="!vuex_isShenHe&&vuex_indexUiType==2">
					...
					<view class="gridSm g3">
						<view class="item" @click="goToGallery()">
							<view class="flex pd10 rds5" style="background-color: rgb(251, 247, 236);">
								<view class="cover">
									<view class="fz16 bold">套图</view>
									<view class="fz12" style="color: rgb(146, 131, 112);">精选朋友圈套图</view>
								</view>
							</view>
						</view>
				...
				<view v-if="UiType==0">
					<view class="u-p-t-30">
						<view class="u-p-l-30 u-p-r-30">
							<u-section title="网友推荐" line-color="#f8c93c" :bold="false" :font-size="36" sub-title="更多"
								@click="listClick('category','',-2)"></u-section>
						</view>
						<scroll-view scroll-x class="u-p-t-30" style="height: 440rpx;">
					...
					<view class="u-p-t-20">
						...
						<view class="u-flex u-row-left u-flex-wrap u-p-l-30 u-p-t-15">
							<block v-for="(item, index) in flowList" :key="index">
								...
								<!-- #ifdef MP-WEIXIN -->
								<view class="u-flex u-row-center u-col-center" style="margin-bottom: 10px;" v-show="adFlag"
									v-if="index != 0 && index%15 === 0">
									<ad :unit-id="vuex_videoUnitId" ad-type="video" ad-theme="white"></ad>
								</view>
								<!-- #endif -->
								<view class="tab" @click="detail(item._id, item.index, item.images)">
				...
				<view v-if="UiType==1">
					<view class="u-p-l-30 u-p-r-30 u-p-t-40 u-p-b-30">
						<u-section title="最新发布" line-color="#f8c93c" :bold="false" :font-size="36" sub-title="更多"
						...
						<u-waterfall v-model="flowList" ref="uWaterfall">
							<template v-slot:left="{ leftList }">
								...
								<view v-for="(item, index) in leftList" :key="item.key">
									<view class="pubucontent" @click="detail(item._id, item.index, item.images)">
							...
							<template v-slot:right="{ rightList }">
								<view v-for="(item, index) in rightList" :key="item.key">
									<view class="contentpubu" @click="detail(item._id, item.index, item.images)">
										<u-lazy-load border-radius="10"
											:image="vuex_compressPicture?item.image+'?x-oss-process=image/resize,w_200/quality,q_80':item.image">
										</u-lazy-load>
									</view>
		...
		methods: {
			async getConfig() {
				...
				let config = await uniCloud.callFunction({
					name: 'config_search',
					data: {
						keys: ['isShenHe', 'uiType', 'appName','wxInterstitiaId']
					}
				});
				if (config.result.success) {
					...
					that.wxInterstitiaId = config.result.data[3]
				}
			},
			async loadAd(){
				var that = this
			...
			async getList(type, more, on) {
				...
				list.result.data.forEach(function(item) {
					item.images.forEach(function(image, _index) {
						newflowList.push({
							'index': _index,
							'image': image,
							'images': item.images,
